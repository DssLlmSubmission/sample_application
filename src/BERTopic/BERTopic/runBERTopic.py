...
class BERTopicAnalysis:
    ...
    def load_data_telegram(self):
        ...
        self.df["messageText"] = self.df["messageText"].apply(lambda x: self.remove_links(x))
        lines = self.df[(self.df['messageText'].str.len() >= 100) & (self.df['messageText'].str.len() <= 500)].messageText.values
...
def main():
    ...
    parser.add_argument('-d', '--data_type', choices=['telegram', 'twitter', 'google_news' ], help='Choose a datasource', required=True)
